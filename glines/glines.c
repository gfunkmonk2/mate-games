...
show_scores (gint pos, gboolean new_game)
{
  ...
  if (dialog == NULL) {
    dialog = games_scores_dialog_new (GTK_WINDOW (app), highscores, _("MATE Five or More"));
...
game_about_callback (GtkAction * action, gpointer * data)
{
  const gchar *authors[] = { "Robert Szokovacs <szo@appaloosacorp.hu>",
    "Szabolcs B\xc3\xa1n <shooby@mate.hu>",
    NULL
  };
...
#endif
                         "version", VERSION,
                         "comments", _("MATE port of the once-popular Color Lines game.\n\nFive or More is a part of MATE Games."),
                         "copyright",
...
main (int argc, char *argv[])
{
  ...
  gtk_window_set_default_icon_name ("mate-glines");
