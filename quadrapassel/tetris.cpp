...
#define TETRIS_OBJECT "matetris-tetris-object"
	...
	dropBlock(false)
{
	...
	const GtkTargetEntry targets[] = {{(gchar*) "text/uri-list", 0, URI_LIST},
					  ...
					  {(gchar*) "x-special/mate-reset-background", 0, RESET}};
...
Tetris::setupPixmap()
{
	...
	if (bgimage && !default_bgimage) {
		...
		/* Note that this heuristic fails for the standard caja
...
Tetris::gameAbout(GtkAction *action, void *d)
{
	...
	const gchar * const authors[] = { "Mate Games Team", NULL };
	...
	gtk_show_about_dialog (GTK_WINDOW (t->getWidget()),
			       ...
			       "logo-icon-name", "mate-quadrapassel",
