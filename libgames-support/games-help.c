                         ...
                         GError **error)
{
  ...
  if (section != NULL) {
    help_uri = g_strdup_printf ("http://library.mate.org/users/%s/stable/%s.html", doc_module, section);
  } else {
    help_uri = g_strdup_printf ("http://library.mate.org/users/%s/stable/", doc_module);
  }
#endif
