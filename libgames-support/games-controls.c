                       ...
                       GamesControlsList *list)
{
  ...
  /* find our mateconf key in the list store and update it */
                                  ...
                                  ...)
{
  ...
  g_return_if_fail (GAMES_IS_CONTROLS_LIST (list));
  g_return_if_fail (first_mateconf_key != NULL);

  va_start (args, first_mateconf_key);

  key = first_mateconf_key;
  while (key) {
...
-add a "Reset to default" button which resets each command to the defaut key
  (the default can be obtained with mateconf_client_get_default_from_schema)
