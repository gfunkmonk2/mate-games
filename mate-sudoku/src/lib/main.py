...
from gtk_goodies import mateconf_wrapper, Undo, dialog_extras
...
class UI (mateconf_wrapper.MateConfWrapper):
    ui = '''<ui>
    ...
    def select_game (self):
        ...
        if choice[0] == game_selector.NewOrSavedGameSelector.SAVED_GAME:
            ...
            self.update_statusbar()
        if self.mateconf['show_toolbar']:
            self.tb.show()
        if self.mateconf['always_show_hints']:
            self.gsd.update_all_hints()
        if self.mateconf['highlight']:
    ...
    def setup_color (self):
        ...
        bgcol = self.mateconf['bg_color']
        if bgcol != '':
            self.gsd.set_bg_color(bgcol)

    def setup_autosave (self):
        gobject.timeout_add_seconds(self.mateconf['auto_save_interval'] or 60, # in seconds...
                            self.autosave)
    ...
    def setup_toggles (self):
        # sync up toggles with mateconf values...
        map(lambda tpl: self.mateconf_wrap_toggle(*tpl),
            [('always_show_hints',
    ...
    def start_worker_thread (self, *args):
        ...
        try:
            if n_new_puzzles < self.mateconf['minimum_number_of_new_puzzles']:
                ...
                self.worker.start()
        except mateconf_wrapper.MateConfError:
    ...
    def resize_cb (self, widget, event):
        self.mateconf['width'] = event.width
        self.mateconf['height'] = event.height
    ...
    def show_about (self, *args):
        ...
        about.set_logo_icon_name("mate-sudoku")
    ...
    def show_help (self, *args):
        try:
            gtk.show_uri(self.w.get_screen(), "ghelp:mate-sudoku", gtk.get_current_event_time())
    ...
    def print_multiple_games (self, *args):
        gp = printing.GamePrinter(self.sudoku_maker, self.mateconf)
        gp.run_dialog()

    @simple_debug
    def generate_puzzle_gui (self, *args):
        sudoku_generator_gui.GameGenerator(self, self.mateconf)
...
def start_game ():
    if options.debug:
        print 'Starting MATE Sudoku in debug mode'
